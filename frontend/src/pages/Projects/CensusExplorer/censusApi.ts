...
function toMetrics(row: string[], geoNameIndex = 0): Metrics {
  ...
  const kids =
    n(row[2]) + n(row[3]) + n(row[4]) + n(row[5]) + // male 3-6
    n(row[14]) + n(row[15]) + n(row[16]) + n(row[17]); // female 27-30 (indexes offset below)

  // Seniors (65+): Male 020-025, Female 044-049
  const seniors =
    n(row[12]) + n(row[13]) + // male 20-21
    n(row[8]) + n(row[9]) + n(row[10]) + n(row[11]) + // male 22-25 (indexes adjusted)
    n(row[20]) + n(row[21]) + n(row[22]) + n(row[23]) + n(row[24]) + n(row[25]); // female 44-49
...
export async function fetchMetricsForState(stateFips: string): Promise<Metrics> {
...
export async function fetchMetricsForCounty(stateFips: string, countyFips: string): Promise<Metrics> {
